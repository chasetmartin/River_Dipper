<script lang="ts">
    ...
    //-----Graph Code ------
        ...
        const yLabel = '↑ cubic feet per second (CFS)'; // a label for the y-axis
        ...
        const xRange: [number, number] = [marginLeft + insetLeft, width - marginRight - insetRight]; // [left, right]
        const yType = scaleLinear; // type of y-scale
        const yRange: [number, number] = [height - marginBottom - insetBottom, marginTop + insetTop]; // [bottom, top]
                ...
                <div class="chart-container">
                    ...
                    >
                      ...
                      <g class="y-axis" transform="translate({marginLeft}, 0)" pointer-events='none'>
                        ...
                        {/each}
                        <text x="-{marginLeft - 50}" y={marginTop - 25}>{yLabel}</text>
                      </g>
                      ...
                      <g class="x-axis" transform="translate(0,{height - marginBottom - insetBottom})" pointer-events='none'>
                        ...
                        {#each xTicks as tick, i}
                          <g class="tick" transform="translate({xScale(tick)}, 0)">
                            ...
                            {/if}
                            <text font-size='11px' x={-marginLeft/4} y="20">{xTicksFormatted[i] + xFormat}</text>
                          </g>
                        {/each}
